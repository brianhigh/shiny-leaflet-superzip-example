...
You can view the progress of the build in the console.
...
You can start your container manually and access your app
    directly:
...
You can stop your container when you are finished using it. Get a
...
Install Java, get ShinyProxy and the `application.yml` file:
...
From the folder containing the ShinyProxy jar file and `application.yml` 
start ShinyProxy.
...
You should now be able to test ShinyProxy at: http://localhost:8080
...
We will use Git to get our example Shiny app. If you have not already
have Git, then [download and
install](https://git-scm.com/book/en/v1/Getting-Started-Installing-Git#Installing-on-Mac)
it.

Install Docker for Mac (stable) from: https://docs.docker.com/docker-for-mac/install/

Once you have opened the DMG file and dragged the Docker app to your
Applications folder, run the Docker app to start Docker.

In Terminal, run the commands to build the docker image if you have not already done so:
...
You can view the progress of the build in Terminal.

**NOTE**: When complete, the container created during this operation
will close. That’s okay.

You can start your container manually and access your app directly:
